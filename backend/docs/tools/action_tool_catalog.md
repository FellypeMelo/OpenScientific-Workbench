...
- **analyze_enzyme_kinetics_assay**(enzyme_name: str, substrate_concentrations, enzyme_concentration: float, modulators: Optional[dict] = None, time_points: Optional[list] = None, output_dir: str = "./") — dose-dependent modulator kinetics. *(Implementation note: `modulators` is a HARD requirement despite the `Optional` type here -- the per-modulator dose-response Hill-curve fit is this tool's only real computation; with no modulator data there is nothing to fit, and refusing beats fabricating a result. `substrate_concentrations` is recorded as the fixed assay condition the titration was measured under, not itself fit.)*
